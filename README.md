...
| `hooks/guard-spiris-apply.py` | PreToolUse Bash guard: blocks prod writes without the `.prod-approved` gate, and `--apply` without a reviewed dry-run. Fails open on error. |
...
- Engine runs under **Python 3.11** (system `python3` is 3.9 and will crash on it).
